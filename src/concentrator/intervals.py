...
class IntervalController(ControllerBase):
    ...
    def __init__(
        ...
        super().__init__(
            delay, 1e-3 * delay, values, on_update, history_length, finish_early
        )
    ...
    def _get_interval(self, index, timestamp):
        ...
        return int(
            round((timestamp - self.shifts[index] - self.origin) / self.interval)
        )
    ...
    def _compute_origin(self, timestamps, valid):
        ...
        self.timer.reset(cothread.Deadline(self.origin + self.interval + self.delay))
        return origin
...
class Controller_extra:
    ...
    def on_update(self, values, valid, origin, timestamps, arrivals):
        ...
        self.offsets_pv.set(1e3 * self.controller.origin_offsets, timestamp=origin)
...
class UpdateWaveform:
    def __init__(self, length):
        ...
        self.severity_wf = numpy.zeros(length, dtype=numpy.uint8) + INVALID_severity
