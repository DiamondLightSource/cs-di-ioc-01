...
class Axis:
    ...
    def __init__(self, parent, axis, length, centre, left, right):
        ...
        self.max_offset = builder.aIn(
            pv_name('OFFSET:MAX'), PREC = 2, EGU = 'um')
        self.min_angle = builder.aIn(
            pv_name('ANGLE:MIN'), PREC = 2, EGU = 'u rad')
        self.max_angle = builder.aIn(
            pv_name('ANGLE:MAX'), PREC = 2, EGU = 'u rad')
    ...
    def update_bcd(self):
        ...
        min_offset, max_offset, min_angle, max_angle = \
            self.compute_limits(offset, angle)
...
class BCD:
    def __init__(self, name, right_id, length, centre):
        ...
        left_id = right_id - 1
        if left_id == 0:   left_id = bpm_list.BPM_count
